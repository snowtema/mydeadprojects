...
import { Link2, Share2, Twitter, Linkedin } from "lucide-react";
...
export function ShareMenu({ url, title, text }: ShareMenuProps) {
  ...
  const btnClass =
    "inline-flex items-center gap-1.5 text-xs px-3 py-1.5 bg-bg-card border border-border rounded-md text-text-muted hover:border-border-hover hover:text-text-dim transition-colors";
    ...
    <div className="flex items-center gap-2 flex-wrap">
      {canShare && (
        <button
          ...
          className={`${btnClass} cursor-pointer sm:hidden`}
          title="Share"
        >
      ...
      <button
        onClick={copyLink}
        className={`${btnClass} cursor-pointer ${canShare ? "hidden sm:inline-flex" : ""}`}
        title="Copy link"
      >
      ...
      <a
        ...
        className={`${btnClass} ${canShare ? "hidden sm:inline-flex" : ""}`}
